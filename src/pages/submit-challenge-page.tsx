      ...
      <div className="relative overflow-hidden border-b border-border bg-jic-forest-light/30">
        ...
        <div className="relative mx-auto max-w-3xl px-4 py-14 sm:px-6 lg:px-8">
          <SectionHeading
            ...
            title="Tell us about the problem in your community"
            description="Add enough context for JIC's AI to understand the issue, check the evidence and identify existing related challenges."
          />
        </div>
      </div>
        ...
        <div className="mt-8 flex items-center justify-between">
          {STEPS.map((label, i) => (
            <div key={label} className="flex flex-1 items-center">
              <div className="flex flex-col items-center gap-1.5">
                ...
                <span
                  className={cn(
                    ...
                    i === step ? "text-jic-forest" : "text-muted-foreground"
                  )}
                >
                  {label}
                </span>
              ...
              {i < STEPS.length - 1 && (
                <div className={cn("mx-2 h-0.5 flex-1", i < step ? "bg-jic-forest" : "bg-border")} />
              )}
            </div>
          ))}
        </div>
        ...
        <div className="mt-10 rounded-2xl border border-border bg-card p-6 sm:p-8">
          ...
          <div className="mt-8 flex items-center justify-between border-t border-border pt-6">
            ...
            {step < STEPS.length - 1 ? (
              <Button
                disabled={!canProceed()}
                className="gap-1.5 bg-jic-deep text-jic-cream hover:bg-jic-deep/90"
                onClick={() => setStep((s) => s + 1)}
              >
  ...
  return (
    <div>
      <div className="mb-1.5 flex items-center justify-between gap-3">
        <label className="block text-sm font-medium text-jic-charcoal">{label}</label>
    ...
    <div className="space-y-5">
      ...
      <Field label="Problem title">
        <Input
          ...
          placeholder="e.g. Frequent hand pump failures in our village"
          maxLength={120}
        />
      </Field>
      ...
      <Field
        ...
        hint="Be specific about what is happening"
      >
        <Textarea
          rows={5}
          value={form.description}
          onChange={(e) => update({ description: e.target.value })}
          placeholder="What is happening? Where does it happen? What happens when people try to use the affected service or infrastructure?"
        />
      </Field>
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        ...
        <Field label="Who is affected?">
          <Input
            ...
            onChange={(e) => update({ affectedGroup: e.target.value })}
            placeholder="e.g. Farming households, schoolchildren"
          />
        </Field>
      </div>

      <div className="grid gap-5 sm:grid-cols-2">
        <Field label="Approximate people affected" hint="An estimate is okay">
          <Input
            type="number"
            min={0}
        ...
        <Field label="How long has this existed?" hint="Important for AI assessment">
          <div className="relative">
            ...
            <Input
              ...
              placeholder="e.g. 3 months / every monsoon"
            />
          </div>
        </Field>
      </div>
      ...
      <Field
        label="What can you actually see or observe?"
        hint="Optional but useful for image validation"
      >
        <Textarea
          rows={3}
          value={form.visibleSymptoms}
          onChange={(e) => update({ visibleSymptoms: e.target.value })}
          placeholder="e.g. broken pipe joint, standing water, cracked road surface, damaged school roof..."
        />
      </Field>

      <div className="rounded-xl border border-jic-forest/15 bg-jic-forest-light/35 p-4">
        <div className="flex items-start gap-3">
          <Sparkles className="mt-0.5 h-4 w-4 shrink-0 text-jic-forest" />
          <div>
            <p className="text-sm font-semibold text-jic-charcoal">Why JIC asks these questions</p>
            <p className="mt-1 text-xs leading-5 text-muted-foreground">
              Your answers help the AI distinguish a genuine community issue from an unclear submission and avoid creating duplicate challenges later.
            </p>
          </div>
        </div>
      </div>
...
function StepLocation({ form, update }: { form: FormData; update: (p: Partial<FormData>) => void }) {
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-jic-forest/15 bg-jic-forest-light/35 p-4">
        <div className="flex items-start gap-3">
          <MapPin className="mt-0.5 h-4 w-4 shrink-0 text-jic-forest" />
          <div>
            <p className="text-sm font-semibold text-jic-charcoal">Location improves duplicate detection</p>
            <p className="mt-1 text-xs leading-5 text-muted-foreground">
              JIC uses location alongside text and evidence to determine whether a similar challenge already exists nearby.
            </p>
          </div>
        </div>
      </div>
      ...
      <Field label="Village / City">
        <Input
          ...
          placeholder="e.g. Kathikund"
        />
      </Field>

      <Field label="GPS location" hint="Optional if you provide a detailed village/city">
        <div className="flex flex-col gap-2 sm:flex-row">
          ...
          <Button
            type="button"
            variant="outline"
            onClick={() => update({ gps: "23.6102° N, 85.2799° E" })}
            className="shrink-0"
          >
...
function StepEvidence({ form, update }: { form: FormData; update: (p: Partial<FormData>) => void }) {
  ...
  const [activeKind, setActiveKind] = useState<EvidenceKind>("image");
  ...
  const addFiles = (files: FileList | null) => {
    ...
    const next: EvidenceFile[] = Array.from(files).map((file, index) => {
      ...
      if (kind === "image" && local.previewUrl) {
        ...
        img.onload = () => {
          update({
            evidenceFiles: form.evidenceFiles.map((entry) =>
              entry.id === local.id ? { ...entry, width: img.naturalWidth, height: img.naturalHeight } : entry
            ),
          });
        };
  ...
  const browse = (kind: EvidenceKind) => {
    ...
    if (inputRef.current) {
      inputRef.current.accept =
        kind === "image" ? "image/*" : kind === "video" ? "video/*" : ".pdf,.doc,.docx,.txt";
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-jic-forest/15 bg-jic-forest-light/35 p-4">
        <div className="flex items-start gap-3">
          <Camera className="mt-0.5 h-4 w-4 shrink-0 text-jic-forest" />
          <div>
            <p className="text-sm font-semibold text-jic-charcoal">Add visual evidence whenever possible</p>
            <p className="mt-1 text-xs leading-5 text-muted-foreground">
              JIC's future vision model can use images and video to understand visible symptoms. Clear, relevant photos make the assessment stronger.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <button
          type="button"
          onClick={() => browse("image")}
          className="flex flex-col items-center gap-2 rounded-xl border border-dashed border-border bg-muted/30 py-8 text-sm font-medium text-muted-foreground transition hover:border-jic-forest hover:bg-jic-forest-light/30 hover:text-jic-forest"
        >
      ...
      {form.evidenceFiles.length > 0 && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <p className="text-sm font-medium text-jic-charcoal">{form.evidenceFiles.length} file(s) attached</p>
            <span className="text-xs text-muted-foreground">AI-ready metadata captured</span>
          </div>
            ...
            {form.evidenceFiles.map((file) => (
              <div key={file.id} className="flex items-center gap-3 rounded-xl border border-border bg-card p-3">
                ...
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-medium text-jic-charcoal">{file.name}</p>
                ...
                <button
                  ...
                  className="flex h-8 w-8 items-center justify-center rounded-lg text-muted-foreground hover:bg-muted hover:text-jic-charcoal"
    ...
    <div className="space-y-5">
      ...
      <Field label="What kind of solution are you looking for?">
        <Textarea
          ...
          placeholder="Describe what success would look like for your community."
        />
      </Field>
    </div>
  );
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="divide-y divide-border rounded-xl border border-border">
        ...
        <ReviewRow label="Observed symptoms" value={form.visibleSymptoms || "—"} />
        <ReviewRow label="Location" value={[form.village, form.block, form.district].filter(Boolean).join(", ") || "—"} />
        <ReviewRow label="Evidence" value={form.evidenceFiles.length ? `${form.evidenceFiles.length} file(s) attached` : "No evidence attached"} />
        <ReviewRow label="Desired outcome" value={form.outcome || "—"} />
      </div>

      <p className="text-xs text-muted-foreground">
        By submitting, you confirm this information is accurate to the best of your knowledge. AI analysis is an assistive review and can request additional evidence.
      </p>
    </div>
  );
}
  ...
  return (
    <div className="mx-auto flex max-w-lg flex-col items-center px-4 py-24 text-center sm:px-6">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        JIC is reading the description, evidence and location before it decides what happens next.
      </p>
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-20 sm:px-6">
      <motion.div
        ...
        className="rounded-3xl border border-jic-forest/20 bg-jic-forest-light/40 p-8 text-center"
      >
