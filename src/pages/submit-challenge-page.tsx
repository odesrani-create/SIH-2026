      ...
      <div className="mx-auto max-w-3xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        {error && (
          <div className="mt-6 flex items-start gap-3 rounded-xl border border-destructive/30 bg-destructive/5 p-4 text-sm text-destructive">
            ...
            <div><p className="font-semibold">Submission failed</p><p className="mt-1">{error}</p></div>
          </div>
        )}
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <div><label className="mb-1.5 block text-sm font-medium text-jic-charcoal">{label}</label>{children}</div>;
}
...
function StepProblem({ form, update }: { form: FormData; update: (p: Partial<FormData>) => void }) {
  return <div className="space-y-5">
    ...
    <Field label="Problem title"><Input value={form.title} onChange={(e) => update({ title: e.target.value })} placeholder="e.g. Frequent hand pump failures in our village" /></Field>
    <Field label="Describe the problem"><Textarea rows={5} value={form.description} onChange={(e) => update({ description: e.target.value })} placeholder="What is happening? How long has this been a problem? What have you tried already?" /></Field>
    ...
    <Field label="Approximate number of people affected"><Input type="number" min={0} value={form.population} onChange={(e) => update({ population: e.target.value })} placeholder="e.g. 1200" /></Field>
  </div>;
}
...
function StepLocation({ form, update }: { form: FormData; update: (p: Partial<FormData>) => void }) {
  const useLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => update({ gps: `${position.coords.latitude.toFixed(6)}, ${position.coords.longitude.toFixed(6)}` }),
      () => update({ gps: "" }),
  ...
  return <div className="space-y-5">
    ...
    <Field label="GPS location"><div className="flex gap-2"><Input value={form.gps} onChange={(e) => update({ gps: e.target.value })} placeholder="Lat, Long (optional)" /><Button type="button" variant="outline" onClick={useLocation}>Use current location</Button></div></Field>
    <div className="flex h-40 items-center justify-center rounded-xl border border-dashed border-border bg-muted/40 text-sm text-muted-foreground">GPS coordinates will be saved with the challenge when provided.</div>
  </div>;
}

function StepEvidence({ form }: { form: FormData; update: (p: Partial<FormData>) => void }) {
  return <div className="space-y-5">
    <p className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-jic-forest"><FileText className="h-3.5 w-3.5" /> Step 3 — Evidence</p>
    <p className="text-sm text-muted-foreground">Evidence storage is the next upload integration. Your challenge can already be submitted without attachments.</p>
    <div className="rounded-xl border border-dashed border-border bg-muted/30 p-8 text-center"><p className="text-sm font-medium">No files attached</p><p className="mt-1 text-xs text-muted-foreground">Photo, video and document upload will be connected to Supabase Storage next.</p></div>
    {form.evidenceFiles.length > 0 && <p className="text-xs text-muted-foreground">{form.evidenceFiles.length} file(s)</p>}
  </div>;
}
...
function StepReview({ form }: { form: FormData }) {
  return <div className="space-y-5">
    <p className="flex items-center gap-1.5 text-xs font-bold uppercase tracking-wide text-jic-forest"><ClipboardList className="h-3.5 w-3.5" /> Step 5 — Review</p>
    <div className="divide-y divide-border rounded-xl border border-border">
      <ReviewRow label="Title" value={form.title || "—"} /><ReviewRow label="Category" value={form.category || "—"} /><ReviewRow label="Affected" value={`${form.affectedGroup || "—"} (~${form.population || "0"} people)`} /><ReviewRow label="Location" value={[form.village, form.block, form.district].filter(Boolean).join(", ") || "—"} /><ReviewRow label="GPS" value={form.gps || "Not provided"} /><ReviewRow label="Desired outcome" value={form.outcome || "—"} />
    </div>
    <p className="text-xs text-muted-foreground">Your submission will be stored in Supabase and assigned a tracking ID after AI analysis.</p>
  </div>;
}

function ReviewRow({ label, value }: { label: string; value: string }) {
  return <div className="flex flex-col gap-0.5 px-4 py-3 sm:flex-row sm:items-start sm:justify-between sm:gap-4"><span className="text-xs font-medium text-muted-foreground">{label}</span><span className="text-sm text-jic-charcoal sm:max-w-sm sm:text-right">{value}</span></div>;
}

function AnalyzingScreen({ checksDone }: { checksDone: number }) {
  return <div className="mx-auto flex max-w-lg flex-col items-center px-4 py-24 text-center sm:px-6"><motion.div animate={{ rotate: 360 }} transition={{ duration: 2, repeat: Infinity, ease: "linear" }} className="flex h-20 w-20 items-center justify-center rounded-full bg-jic-forest-light"><Sparkles className="h-9 w-9 text-jic-forest" /></motion.div><h2 className="mt-8 text-2xl font-bold text-jic-charcoal">Analysing your challenge…</h2><p className="mt-2 text-sm text-muted-foreground">AI is evaluating your submission before it is saved.</p><div className="mt-8 w-full space-y-3 text-left">{ANALYSIS_CHECKS.map((label, i) => <div key={label} className="flex items-center gap-3 text-sm">{i < checksDone ? <CheckCircle2 className="h-5 w-5 text-jic-forest" /> : <div className="h-5 w-5 rounded-full border-2 border-border" />}<span className={i < checksDone ? "text-jic-charcoal" : "text-muted-foreground"}>{label}</span></div>)}</div></div>;
}
