    ...
    <>
      ...
      >
        <div className="mx-auto flex h-16 min-w-0 max-w-[1400px] items-center justify-between gap-2 px-3 sm:px-6 lg:px-8">
          <button onClick={() => goTo("landing")} className="group flex min-w-0 items-center gap-2.5">
            ...
            <span className="hidden truncate font-display text-[15px] font-semibold leading-none tracking-tight text-jic-charcoal min-[420px]:block">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <Button
              ...
              onClick={() => goTo("submit")}
            >
              <span className="hidden sm:inline">Submit a Challenge</span>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-50 border-t border-border bg-background/95 backdrop-blur-md lg:hidden">
        <div className="mx-auto grid max-w-md grid-cols-3 gap-2 px-3 py-2">
          {[
            { label: "Home", page: "landing" as PageId },
            { label: "Challenges", page: "challenges" as PageId },
            { label: "Submit", page: "submit" as PageId },
          ].map((item) => (
            <button
              key={item.page}
              onClick={() => {
              ...
              className={cn(
                ...
                nav.page === item.page ? "bg-jic-deep text-jic-cream" : "bg-muted text-jic-charcoal"
              )}
            >
              {item.label}
            </button>
