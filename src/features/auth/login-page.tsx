...
export function LoginPage() {
  ...
  return (
    <div className="grid min-h-[calc(100vh-68px)] lg:grid-cols-2">
      ...
      <div className="flex items-center justify-center bg-jic-cream px-4 py-8 sm:px-6 lg:bg-background lg:px-12 lg:py-14">
        <div className="w-full max-w-md">
          <div className="mb-6 flex items-center gap-2.5 lg:hidden"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-jic-deep text-jic-saffron"><Sprout className="h-5 w-5" /></span><span className="font-display text-base font-semibold text-jic-charcoal">Jharkhand Innovation Connect</span></div>
          <h2 className="font-display text-3xl font-semibold text-jic-charcoal sm:text-4xl lg:text-2xl">{title}</h2><p className="mt-1.5 text-sm text-muted-foreground">{mode === "forgot" ? "Enter your email and we will send reset instructions." : "Join Jharkhand's community problem-solving network."}</p>
          {mode === "signin" && <div className="mt-5 rounded-lg border border-border bg-card p-3"><p className="text-xs font-semibold text-jic-charcoal">Demo access</p><div className="mt-2 grid grid-cols-2 gap-1.5 sm:grid-cols-3">{DEMO_ACCOUNTS.map((account) => <button key={account.email} type="button" onClick={() => { setEmail(account.email); setPassword(account.password); setRole(account.role); }} className="rounded border border-border px-2 py-1.5 text-left text-[11px] font-semibold text-muted-foreground hover:border-jic-forest hover:text-jic-forest">{account.role}</button>)}</div></div>}
          {mode !== "forgot" && <div className="my-5 flex items-center gap-3 text-[11px] text-muted-foreground sm:text-xs"><span className="h-px flex-1 bg-border" /> or continue with email <span className="h-px flex-1 bg-border" /></div>}
